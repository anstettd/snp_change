...
snp_prop_A<- env1_base %>% select (chr_snp)
counter=2
pop_num=1
for (i in seq(1,dim(env1_base)[2]-1,2)) {
  
  snpA<-paste("V",i, sep="") #sets up string for snpA for pop num
  P<-paste("P", pop_num, sep="") #sets up paper_ID (population ID)
  tmp<-env1_base %>% select(snpA) #graps all snps per paper_ID
...
snp_prop_B<- env1_base %>% select (chr_snp)
counter=2
pop_num=1
for (i in seq(1,dim(env1_base)[2]-1,2)) {
  ...
  P<-paste("P", pop_num, sep="") #sets up paper_ID (population ID)
  tmp<-env1_base %>% select(snpB) #graps all snps per paper_ID
...
for (i in 1:dim(snp_prop_A)[1]) { #for each SNP
  ...
  env_pop <- climate %>% select(Paper_ID,MAT) #Select population ID and climate data
  ...
  #This is the glm where we associate the A and B SNPs with climate, in this case it's MAT
  ...
  if(inv.logit(lm.temp_A$coefficients[2])>0.5){ 
    ...
    tmp_in<-env_pop %>% select(Paper_ID, prop_A, prop_B) %>% 
      mutate (ENV="MAT", chr_snp=snp_prop_A$chr_snp[i], SNP_Select="A", 
    ...
    tmp_in<-env_pop %>% select(Paper_ID, prop_B, prop_A) %>% 
      mutate (ENV="MAT", chr_snp=snp_prop_A$chr_snp[i], SNP_Select="B",
...
for (i in 1:dim(freq.temp)[1]){
  Binomial_A<-c(rep(1, freq.temp[i, "True_SNP_A"]), rep(0,freq.temp[i, "True_SNP_B"]))
  tmp_df<- as.data.frame(Binomial_A) %>% mutate (Paper_ID=freq.temp$Paper_ID[i], 
                                                 chr_snp=freq.temp$chr_snp[i], 
                                                 ENV=freq.temp$ENV[i])
