...
colnames(baseline_snp_slope) <- c("ENV","chr_snp","Slope_base","Inv_Logit_Coeff","SE_base")

baseline_snp_slope_env1 <- baseline_snp_slope %>% filter(ENV=="MAT")
baseline_snp_slope_env2 <- baseline_snp_slope %>% filter(ENV=="MAP")
baseline_snp_slope_env3 <- baseline_snp_slope %>% filter(ENV=="PAS")
baseline_snp_slope_env4 <- baseline_snp_slope %>% filter(ENV=="EXT")
baseline_snp_slope_env5 <- baseline_snp_slope %>% filter(ENV=="CMD")
baseline_snp_slope_env6 <- baseline_snp_slope %>% filter(ENV=="Tave_wt")
baseline_snp_slope_env7 <- baseline_snp_slope %>% filter(ENV=="Tave_sm")
baseline_snp_slope_env8 <- baseline_snp_slope %>% filter(ENV=="PPT_wt")
baseline_snp_slope_env9 <- baseline_snp_slope %>% filter(ENV=="PPT_sm")

#Add percentile
baseline_snp_slope_env1 <- baseline_snp_slope_env1 %>% mutate(percentile=ecdf(Inv_Logit_Coeff)(Inv_Logit_Coeff))
baseline_snp_slope_env2 <- baseline_snp_slope_env2 %>% mutate(percentile=ecdf(Inv_Logit_Coeff)(Inv_Logit_Coeff))
baseline_snp_slope_env3 <- baseline_snp_slope_env3 %>% mutate(percentile=ecdf(Inv_Logit_Coeff)(Inv_Logit_Coeff))
baseline_snp_slope_env4 <- baseline_snp_slope_env4 %>% mutate(percentile=ecdf(Inv_Logit_Coeff)(Inv_Logit_Coeff))
baseline_snp_slope_env5 <- baseline_snp_slope_env5 %>% mutate(percentile=ecdf(Inv_Logit_Coeff)(Inv_Logit_Coeff))
baseline_snp_slope_env6 <- baseline_snp_slope_env6 %>% mutate(percentile=ecdf(Inv_Logit_Coeff)(Inv_Logit_Coeff))
baseline_snp_slope_env7 <- baseline_snp_slope_env7 %>% mutate(percentile=ecdf(Inv_Logit_Coeff)(Inv_Logit_Coeff))
baseline_snp_slope_env8 <- baseline_snp_slope_env8 %>% mutate(percentile=ecdf(Inv_Logit_Coeff)(Inv_Logit_Coeff))
baseline_snp_slope_env9 <- baseline_snp_slope_env9 %>% mutate(percentile=ecdf(Inv_Logit_Coeff)(Inv_Logit_Coeff))



#Merge slopes from baseline populations with timeseries slopes
slope_base_env1 <- left_join(slope_env1,baseline_snp_slope_env1,by=c("snp_ID"="chr_snp"))
slope_base_env2 <- left_join(slope_env2,baseline_snp_slope_env2,by=c("snp_ID"="chr_snp"))
slope_base_env3 <- left_join(slope_env3,baseline_snp_slope_env3,by=c("snp_ID"="chr_snp"))
slope_base_env4 <- left_join(slope_env4,baseline_snp_slope_env4,by=c("snp_ID"="chr_snp"))
slope_base_env5 <- left_join(slope_env5,baseline_snp_slope_env5,by=c("snp_ID"="chr_snp"))
slope_base_env6 <- left_join(slope_env6,baseline_snp_slope_env6,by=c("snp_ID"="chr_snp"))
slope_base_env7 <- left_join(slope_env7,baseline_snp_slope_env7,by=c("snp_ID"="chr_snp"))
slope_base_env8 <- left_join(slope_env8,baseline_snp_slope_env8,by=c("snp_ID"="chr_snp"))
slope_base_env9 <- left_join(slope_env9,baseline_snp_slope_env9,by=c("snp_ID"="chr_snp"))

slope_env_all <- rbind(slope_base_env1,
                       slope_base_env2,
                       slope_base_env3,
                       slope_base_env4,
                       slope_base_env5,
                       ...
                       slope_base_env8,
                       slope_base_env9)
slope_env_all <- na.omit(slope_env_all)
...
###################################################################################
#Export
write_csv(slope_env_all_unique, "data/binomial_data_half/slope_obs_all_unique_base_slope.csv")
