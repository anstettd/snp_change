...
#Import files
env_obs_ci_2unique <- read_csv("data/binomial_data/obs_ci_env_unique.csv")

env_obs_ci_unique <-env_obs_ci_2unique %>% mutate(S=S/2)
...
histPop
ggsave("graphs/01_slope_ci_histPop_abs.pdf",width=12, height = 8, units = "in")
...
histPop <- ggplot(env_histPop_25 ,aes(x=S,y=obs,ymin=low,ymax=high))+
  ...
  theme_ci() + facet_wrap(.~pop_lable)
histPop
ggsave("graphs/02_slope_ci_histPop_2.5_abs.pdf",width=12, height = 8, units = "in")
