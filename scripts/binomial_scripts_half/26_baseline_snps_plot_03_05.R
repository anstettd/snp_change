...
abund_env1_rm <- abund_env1 %>% group_by(chr_snp) %>% summarize(avg_slope = mean(Binomial_A)) %>% 
  filter(avg_slope<0.95) %>% filter(avg_slope>0.05) 
...
ggsave("graphs/spaghetii/base_slope_cutoff/8_03_spaghetii_baseline_env_05_extreme_cuttoff.pdf",arrg_1,width=8, height = 7, units = "in")
