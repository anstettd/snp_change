...
obs_env_unique <- read_csv("data/binomial_bf30/slope_obs_all_unique.csv") %>% 
  filter(SE<5.5) %>% mutate(abs_slope = abs(Slope))

setwd("/Users/daniel_anstett/Dropbox/AM_Workshop/Large_files")
rand_env_unique <- read_csv("rand_slope_histPop_lowSE_bf30.csv")
...
write_csv(wilcox.out, "data/binomial_bf30/wilcox_S.csv")
...
write_csv(emp_out, "data/binomial_bf30/mean_median_S.csv")
...
ggsave("graphs/mean_median_s/5_bf30_median.pdf",width=12, height = 8, units = "in")
...
ggsave("graphs/mean_median_s/6_bf30_wza_mean.pdf",width=12, height = 8, units = "in")
