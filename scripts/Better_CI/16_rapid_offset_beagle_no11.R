...
offset_pop <- offset_pop %>% filter(Paper_ID!=11)
...
ggsave("graphs/bi_plots_95/9_rapid_offset_ssp245_all.pdf",width=7, height = 6, units = "in")
...
ggplot(offset_pop, aes(x=offset_SSP585, y=cumul_all, label=Paper_ID)) + 
  ...
  theme_classic() + theme(
    ...
    legend.position = c(0.65, 0.85),legend.text=element_text(size=14),
    legend.title=element_text(size=16,face="bold"))
ggsave("graphs/bi_plots_95/10_rapid_offset_ssp585_all.pdf",width=7, height = 6, units = "in")
