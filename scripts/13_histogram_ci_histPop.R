...
#Import files
env_obs_ci_unique <- read_csv("data/obs_ci_env_unique.csv")
...
histPop
ggsave("graphs/01_slope_ci_histPop.pdf",width=12, height = 8, units = "in")
...
histPop <- ggplot(env_histPop_25 ,aes(x=S,y=obs,ymin=low,ymax=high))+
  ...
  theme_ci() + facet_wrap(.~pop_lable)
histPop
ggsave("graphs/02_slope_ci_histPop_2.5.pdf",width=12, height = 8, units = "in")
