##################################################################################
## Statistical tests for median abs slope
...
rand_env_unique <- read_csv("rand_slope_histPop_lowSE_ci.csv") %>% mutate(abs_slope=abs(Slope))
setwd("~/Dropbox/AM_Workshop/snp_change")

#Get slope median
median_obs <- obs_env_unique %>% group_by(Site) %>% summarise(median = median(abs_slope, na.rm = TRUE))
median_rand <- rand_env_unique %>% group_by(Site,Seed_ID) %>% summarise(median = median(abs_slope, na.rm = TRUE))

mean_obs <- obs_env_unique %>% group_by(Site) %>% summarise(mean = mean(abs_slope, na.rm = TRUE))
mean_rand <- rand_env_unique %>% group_by(Site,Seed_ID) %>% summarise(mean = mean(abs_slope, na.rm = TRUE))


###################################################################################
#Test median and mean abs_slope is different from zero
...
for (i in 1:12){
  ...
  wilcox.out[i,4] <- wilcox.test(obs_test$abs_slope, mu = 0)$p.value
}

colnames(wilcox.out) <- c("Site","Median","Mean","p-value")


#write_csv(wilcox.out, "data/binomial_data_half/wilcox_S_abs.csv")
...
#write_csv(emp_out, "data/binomial_data_half/mean_median_S_abs.csv")
...
ggsave("graphs/mean_median_s/median_abs.pdf",width=12, height = 8, units = "in")
...
ggsave("graphs/mean_median_s/mean_abs.pdf",width=12, height = 8, units = "in")
