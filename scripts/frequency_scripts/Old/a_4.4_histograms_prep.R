...
get_range <- function(df,site){
  ...
  #Get ranges  
  env_range1 <- env_slope %>% filter(Slope >= -2.6 & Slope< -2.4)
  env_range2 <- env_slope %>% filter(Slope >= -2.4 & Slope < -2.2)
  env_range3 <- env_slope %>% filter(Slope >= -2.2 & Slope < -2)
  env_range4 <- env_slope %>% filter(Slope >= -2 & Slope< -1.8)
  env_range5 <- env_slope %>% filter(Slope >= -1.8 & Slope < -1.6)
  env_range6 <- env_slope %>% filter(Slope >= -1.6 & Slope < -1.4)
  env_range7 <- env_slope %>% filter(Slope >= -1.4 & Slope < -1.2)
  env_range8 <- env_slope %>% filter(Slope >= -1.2 & Slope < -1)
  env_range9 <- env_slope %>% filter(Slope >= -1 & Slope < -0.8)
  env_range10 <- env_slope %>% filter(Slope >= -0.8 & Slope < -0.6)
  env_range11 <- env_slope %>% filter(Slope >= -0.6 & Slope < -0.4)
  env_range12 <- env_slope %>% filter(Slope >= -0.4 & Slope < -0.2)
  env_range13 <- env_slope %>% filter(Slope >= -0.2 & Slope < 0)
  env_range14 <- env_slope %>% filter(Slope >= 0 & Slope < 0.2)
  env_range15 <- env_slope %>% filter(Slope >= 0.2 & Slope < 0.4)
  env_range16 <- env_slope %>% filter(Slope >= 0.3 & Slope < 0.6)
  env_range17 <- env_slope %>% filter(Slope >= 0.4 & Slope < 0.8)
  env_range18 <- env_slope %>% filter(Slope >= 0.8 & Slope < 1)
  env_range19 <- env_slope %>% filter(Slope >= 1 & Slope < 1.2)
  env_range20 <- env_slope %>% filter(Slope >= 1.2 & Slope < 1.4)
  env_range21 <- env_slope %>% filter(Slope >= 1.4 & Slope < 1.6)
  env_range22 <- env_slope %>% filter(Slope >= 1.6 & Slope < 1.8)
  env_range23 <- env_slope %>% filter(Slope >= 1.8 & Slope < 2)
  env_range24 <- env_slope %>% filter(Slope >= 2 & Slope < 2.2)
  env_range25 <- env_slope %>% filter(Slope >= 2.2 & Slope < 2.4)
  env_range26 <- env_slope %>% filter(Slope >= 2.4 & Slope < 2.6)
...
mat_slope <- read_csv("Genomics_scripts/Data/freq_MAT_slope_peakbf5.csv")
map_slope <- read_csv("Genomics_scripts/Data/freq_MAP_slope_peakbf5.csv")
cmd_slope <- read_csv("Genomics_scripts/Data/freq_cmd_slope_peakbf5.csv")

setwd("/Users/daniel_anstett/Dropbox/AM_Workshop/Large_files")

#Import large files with 1000 X random slopes
mat_bars <- read_csv("rand_slope_mat_multi_peakbf5.csv")
map_bars <- read_csv("rand_slope_map_multi_peakbf5.csv")
cmd_bars <- read_csv("rand_slope_cmd_multi_peakbf5.csv")
...
mat_slope2 <- mat_slope %>% filter(Slope<2.6 & Slope>-2.6) 
map_slope2 <- map_slope %>% filter(Slope<2.6 & Slope>-2.6) 
cmd_slope2 <- cmd_slope %>% filter(Slope<2.6 & Slope>-2.6) 

mat_bars2 <- mat_bars %>% filter(Slope<2.6 & Slope>-2.6) 
map_bars2 <- map_bars %>% filter(Slope<2.6 & Slope>-2.6) 
cmd_bars2 <- cmd_bars %>% filter(Slope<2.6 & Slope>-2.6) 

#Setup data frame
env_obs <- as.data.frame(c(-2.5,-2.3,-2.1,-1.9,-1.7,-1.5,-1.3,-1.1,-0.9,-0.7,-0.5,-0.3,-0.1,
...
for(j in 1:12){
  for(i in 0:25){
    ...
    mat_test <- mat_bars %>% filter(Slope >= (-2.6+0.2*i) & Slope< (-2.4+0.2*i))
...
for(j in 1:12){
  for(i in 0:25){
    ...
    map_test <- map_bars %>% filter(Slope >= (-2.6+0.2*i) & Slope< (-2.4+0.2*i))
...
for(j in 1:12){
  for(i in 0:25){
    ...
    cmd_test <- cmd_bars %>% filter(Slope >= (-2.6+0.2*i) & Slope< (-2.4+0.2*i))
...
#Export
write_csv(mat_obs_ci, "Genomics_scripts/Data/mat_obs_ci_peakbf5.csv")
write_csv(map_obs_ci, "Genomics_scripts/Data/map_obs_ci_peakbf5.csv")
write_csv(cmd_obs_ci, "Genomics_scripts/Data/cmd_obs_ci_peakbf5.csv")
write_csv(env_obs_ci, "Genomics_scripts/Data/env_obs_ci_peakbf5.csv")
